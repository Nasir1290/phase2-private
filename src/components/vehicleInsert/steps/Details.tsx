...
export function DetailsStep({ errors }: DetailsStepProps) {
  ...
  return (
    <div>
      <VehicleInsertionHeader title="Descrizione" subtitle="Fai risaltare il tuo annuncio con una descrizione dettagliata" />
      {/* vehicle Description  */}
      <div className="space-y-4">
        ...
        <p className="text-sm text-text_light_gray font-medium text-center mr-14 2xl:mr-28">
          {Math.min(formData.description?.trim().split(/\s+/).filter(Boolean).length || 0, 20)}
          /20 Parole
        </p>
      </div>
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              if (value === "" || (!isNaN(Number(value)) && Number(value) >= 0)) {
                handleInputChange("deposite", value === "" ? null : Number(value));
              }
            }}
            onKeyDown={(e) => {
            ...
            className={`relative p-5 h-12 max-w-72 mt-2 border border-black/5 shadow-md rounded-md ${
              hasError("deposite") && !formData.deposite ? "border-primary" : "border-gray-100"
            }`}
          />
          <p className="absolute top-[38px] left-[230px] font-medium text-[15px] bg-white p-2">CHF</p>
        </div>
        {/* Textareas */}
        <div className="grid grid-cols-2 gap-10">
          <div className="space-y-3">
            ...
            <Textarea
              ...
              onChange={(e) => handleInputChange("depositePolicy", e.target.value)}
              className={`${
                hasError("depositePolicy") && !formData.depositePolicy ? "border-primary" : "border-gray-100"
          ...
          <div className="space-y-3">
            ...
            <Textarea
              ...
              className={`p-5 h-36 resize-none max-w-[580px] shadow-lg border border-black/5 rounded-lg ${
                hasError("fuelPolicy") && !formData.fuelPolicy ? "border-primary" : "border-gray-100"
              }`}
            />
          </div>
          ...
          <div className="space-y-3">
            ...
            <Textarea
              ...
              className={`p-5 h-36 resize-none max-w-[580px] shadow-lg border border-black/5 rounded-lg ${
                hasError("mileagePolicy") && !formData.mileagePolicy ? "border-primary" : "border-gray-100"
              }`}
            />
          </div>
          ...
          <div className="space-y-3">
            ...
            <Textarea
              ...
              className={`p-5 h-36 resize-none max-w-[580px] shadow-lg border border-black/5 rounded-lg ${
                hasError("damagePolicy") && !formData.damagePolicy ? "border-primary" : "border-gray-100"
              }`}
            />
          </div>
        </div>
