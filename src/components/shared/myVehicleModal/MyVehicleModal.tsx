...
const MyVehicleModal: React.FC<MyVehicleModalProps> = ({
  ...
  const handleStatusChange = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.error(
          `Failed to update car status: ${res.message || "Please try again."}`
        );
      }
  ...
  return (
    <div
      className="fixed inset-0 bg-footer_bg/50 flex justify-center items-center z-50 overflow-auto py-4"
      onClick={toggleModal}
    >
      <div
        className="bg-white p-4 md:px-9 md:py-7 rounded-xl w-full max-w-[600px] mx-4 relative"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={toggleModal}
          className="absolute top-3 right-5 text-3xl text-text_light_gray hover:text-red"
        >
          &times;
        </button>
        <div className="space-y-4">
          <div className="flex flex-col items-start gap-3">
            <h2 className="text-xl md:text-[22px] font-semibold text-start md:mb-5">
              {title}
            </h2>
            <div className="flex flex-col sm:flex-row items-start gap-6">
              ...
              <div className="space-y-4 flex-1 min-w-0">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="font-medium text-base md:text-xl truncate">
                    {brand + " " + model}
                  </h2>
                </div>
                <p className="text-sm text-text_light_gray break-words">
          ...
          <div className="mt-4 flex justify-between gap-3">
            ...
            <button
              ...
              className="w-36 bg-red text-white text-center py-2 shadow-lg text-sm px-4 rounded-md flex items-center justify-center space-x-2 font-medium"
            >
              {confirmButtontext}
            </button>
          </div>
