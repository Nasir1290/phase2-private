...
const DetailFaq = () => {
  const faqData = [
    {
      question:
        "Come funziona il servizio di riconsegna flessibile del veicolo?",
      answer:
    ...
    {
      question:
        "Come posso contattare il servizio clienti in caso di emergenza?",
      answer:
    ...
    },
    {
      ...
      answer:
        "Sì, l’assicurazione casco totale è inclusa per tutto il periodo di noleggio.",
    },
  ];
      ...
      <Accordion type="single" collapsible className="space-y-5 mb-10">
        {faqData.map((item, index) => (
          <AccordionItem value={`item-${index}`} key={index}>
            ...
            <AccordionContent className="p-3 rounded-b-lg text-sm  bg-section_bg/20 ">
              {item.answer}
            </AccordionContent>
          </AccordionItem>
        ))}
