    ...
    <div className="w-full">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-2 justify-items-end gap-8">
            ...
            <header className="mt-2 xl:mt-10 col-span-1 xl:col-span-1 2xl:col-span-1 text-start mb-8 lg:mb-0">
              <div className="flex flex-col gap-4">
                <span className="text-sm font-medium text-white uppercase">NETWORK</span>
                <h1 className=" text-3xl md:text-[30px] font-extrabold text-white">
                  Registra <br />
                  <span className="text-primary">la tua flotta di veicoli</span> <br />
                  su bittengo.org
                </h1>
            ...
            <div className="col-span-1 bg-section_bg xl:bg-section_bg/85 shadow w-full xl:max-w-md mx-auto lg:mx-0 rounded-2xl px-10 py-6 hidden md:flex flex-col items-center">
              <div className="flex items-center justify-between mb-6 w-full ">
                <h2 className="text-base lg:text-lg 2xl:text-2xl font-bold">Registrazione Gratuita</h2>
                <Image src={registrationKey || "/placeholder.svg"} alt="check" width={40} height={40} className="w-8 h-8 2xl:w-10 2xl:h-10" />
              </div>
              <div className="space-y-4 2xl:space-y-5">
                {[
                  ...
                  <div key={index} className="flex items-start gap-6">
                    <Image src={check || "/placeholder.svg"} alt="check" width={20} height={20} className="w-4 h-4 flex-shrink-0" />
              ...
              <div className="mx-auto">
                <Link href="/register">
                  <button
                    // onClick={toggleModal}
                    className="bg-primary text-white px-6 py-2.5 rounded-lg text-[15px] font-medium w-72"
                  >
                    INIZIA ORA
                  </button>
                </Link>
      ...
      <div className="md:hidden col-span-1 bg-gray-200/70 xl:bg-section_bg/85 shadow w-full  mx-auto lg:mx-0 rounded-2xl p-4 flex flex-col items-center">
        ...
        >
          <div className="flex items-center justify-between mb-6 w-full">
            <h2 className="text-base lg:text-lg 2xl:text-2xl font-bold">Registrazione Gratuita</h2>
            <Image src={registrationKey} alt="check" width={40} height={40} className="w-8 h-8 2xl:w-10 2xl:h-10" />
          </div>
          ...
          {[
            ...
            <div key={index} className="flex items-start gap-6">
              <Image src={check} alt="check" width={20} height={20} className="w-4 h-4 flex-shrink-0" />
              <p className="text-sm">{text}</p>
            </div>
          ))}
        </motion.div>
        ...
        <div className="mt-5 bg-white mx-auto flex flex-col items-center text-text_light_gray font-normal justify-center gap-3 p-4 rounded-lg">
          ...
          <Link
            href="/register"
            className="bg-primary text-white px-6 py-2.5 rounded-lg text-[15px] font-medium w-80 flex items-center justify-center"
          >
            INIZIA ORA
          </Link>
        </div>
      ...
      <div className="hidden md:block bg-white px-16 xl:px-4 py-4 sm:py-6 shadow-lg ">
        <div className="max-w-[1200px] mx-auto flex flex-wrap justify-between gap-4">
          {["Vantaggi", "Clientela", "Termini e condizioni", "FAQ"].map((item, index) => (
            <Link
              key={index}
              href={`#partnership-${item.toLowerCase().replace(/\s+/g, "-")}`}
              className="font-medium underline hover:text-primary text-sm sm:text-base"
            >
              {item}
            </Link>
          ))}
