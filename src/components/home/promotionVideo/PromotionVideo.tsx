    ...
    <div className="bg-[#F1F1F1] py-20">
      ...
      <div className="container mx-auto flex flex-col lg:flex-row items-start lg:items-center justify-between gap-5 ">
        <div className="col-span-12 md:col-span-6">
          <p className="text-sm font-semibold text-black uppercase mb-5">SOCIAL MEDIA</p>
          <div className="w-full md:w-[420px] lg:max-w-96">
            <SectionHeader2 title="Resta aggiornato sulle" highlightedText="prossime promozioni" />
          </div>
        </div>
        ...
        <div className="col-span-12 md:col-span-6 flex justify-between lg:justify-end items-center md:gap-4 lg:gap-10 xl:gap-12 w-full lg:w-2/3">
          ...
          <div className="flex gap-2 md:gap-4 lg:gap-6">
            <Link href="https://www.instagram.com/bittengo_official?igsh=MW5tMnExb2FsOGxuaw%3D%3D&utm_source=qr">
              <Image src={icon3} alt="Instagram" width={40} height={40} className="w-7 h-7 md:w-10 md:h-10" />
            </Link>
            <Link href="https://www.facebook.com/people/Bittengo/61557722955719/?mibextid=LQQJ4d">
              <Image src={icon1} alt="Facebook" width={40} height={40} className="w-7 h-7 md:w-10 md:h-10" />
            </Link>
            <Link href="https://www.linkedin.com/company/bittengo">
              <Image src={icon2} alt="Linkedin" width={40} height={40} className="w-7 h-7 md:w-10 md:h-10" />
            </Link>
          </div>
        </div>
      </div>
      ...
      <div className="mt-10 container mx-auto overflow-x-auto scrollbar-hide">
        <div className="flex gap-4 md:gap-5 xl:gap-[60px] 2xl:gap-[70px] mr-10">
          {videos.map((video) => (
            <div key={video.id} className="relative min-w-[200px] md:min-w-[250px]">
              {/* Video Thumbnail */}
              <Image
                src={video.thumbnail}
                ...
                width={250}
                height={250}
                className="relative rounded-xl cursor-pointer h-[330px] sm:h-[350px] md:h-[380px] 2xl:h-[450px] bg-primary w-[200px] md:w-full"
                onClick={() => handlePlayClick(video.videoUrl)}
              />
              {/* Play Button */}
              <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                <button className="bg-primary rounded-full p-4 shadow-lg" onClick={() => handlePlayClick(video.videoUrl)}>
      ...
      {selectedVideo && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 ">
          <div ref={modalRef} className="relative p-4 rounded-lg mx-auto max-w-[500px]">
            {/* Close Button */}
            <button
            ...
            {/* Video Player */}
            <video controls autoPlay className="w-full h-[600px] md:h-[700px] lg:h-[750px] xl:h-[750px] 2xl:h-[800px] object-cover rounded-xl ">
